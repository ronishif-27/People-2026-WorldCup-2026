...
export default function Leaderboard({ employees, userId, activityLogs }: LeaderboardProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* 3. Leaderboard Grid/Table — 6 columns: Rank | Profile | Site & Dept | Games | Wins | Coins */}
      <div className="bg-white rounded-3xl border border-[#001529]/10 shadow-sm overflow-hidden">
        ...
        <div className="hidden md:grid grid-cols-12 gap-4 px-6 py-4 bg-slate-50 border-b border-slate-100 text-[10px] font-black text-slate-400 uppercase tracking-widest">
          ...
          <div className="col-span-4">Employee &amp; Profile</div>
          <div className="col-span-3">Site &amp; Department</div>
          <div className="col-span-1 text-center">Games</div>
          <div className="col-span-1 text-center">Wins</div>
          <div className="col-span-2 text-right">Coins Balance</div>
        </div>
          ...
          <AnimatePresence initial={false}>
            {visibleEmployees.length === 0 ? (
              ...
              visibleEmployees.map((emp) => {
                ...
                // Server-authoritative rank (employees prop is already sorted by server)
                const globalRank = employees.findIndex(e => e.id === emp.id) + 1;
                const totalGames = emp.totalGames ?? 0;
                const totalWins  = emp.totalWins  ?? 0;

                return (
                  <motion.div
                    layout
                    key={emp.id}
                    className={`grid grid-cols-1 md:grid-cols-12 gap-2 md:gap-4 items-center px-6 py-4 transition-all duration-300 ${
                      isUser
                  ...
                  >
                    ...
                    <div className="col-span-1 flex items-center gap-2 md:justify-center">
                      <span className="md:hidden text-[10px] font-black text-slate-400 uppercase">Rank:</span>
                      {globalRank === 1 ? (
                        <div className="w-7 h-7 rounded-xl bg-amber-400 flex items-center justify-center text-white text-sm shadow-sm font-black border border-amber-300 animate-bounce">🥇</div>
                      ) : globalRank === 2 ? (
                        <div className="w-7 h-7 rounded-xl bg-slate-300 flex items-center justify-center text-white text-sm shadow-sm font-black border border-slate-200">🥈</div>
                      ) : globalRank === 3 ? (
                        <div className="w-7 h-7 rounded-xl bg-amber-600 flex items-center justify-center text-white text-sm shadow-sm font-black border border-amber-500">🥉</div>
                      ) : (
                        <span className="font-mono text-slate-500 font-black text-sm pl-2">{globalRank}</span>
                      )}
                    </div>
                    ...
                    <div className="col-span-4 flex items-center gap-3">
                      {emp.avatarUrl ? (
                        ...
                        <div className={`w-9 h-9 rounded-full flex items-center justify-center bg-gradient-to-tr ${emp.avatarColor || 'from-slate-500 to-slate-700'} text-white font-black text-xs shadow-xs shrink-0`}>
                          {initials}
                        </div>
                      )}
                      <div className="truncate">
                        <span className="font-extrabold text-slate-800 flex items-center gap-1.5 text-sm">
                          {emp.fullName}
                          {isUser && (
                            <span className="bg-[#14665F] text-white text-[9px] font-black px-2 py-0.5 rounded-full uppercase tracking-widest shadow-xs">You</span>
                          )}
                        </span>
                        {/* Mobile: site + dept stacked under name */}
                        <div className="flex md:hidden items-center gap-2 mt-1 text-[10px] font-bold text-slate-500">
                          <span className="bg-slate-100 px-1.5 py-0.5 rounded text-[9px] uppercase tracking-wider text-[#14665F]">{emp.department}</span>
                          <span>•</span>
                          <span className="bg-slate-100 px-1.5 py-0.5 rounded text-[9px] uppercase tracking-wider flex items-center gap-0.5">
                            <MapPin className="w-3 h-3 text-slate-400" /> {emp.site}
                          </span>
                        </div>
                      </div>
                    </div>

                    {/* Site & Department (desktop) */}
                    <div className="hidden md:flex md:col-span-3 flex-col gap-0.5 min-w-0">
                      <span className={`font-bold text-xs uppercase tracking-wider truncate ${isUser ? "text-[#14665F]" : "text-slate-700"}`}>
                        {emp.department || '—'}
                      </span>
                      <span className="text-slate-500 font-bold text-[11px] flex items-center gap-1 uppercase tracking-wider truncate">
                    ...
                    {/* Total Games */}
                    <div className="col-span-1 flex md:justify-center justify-between items-center bg-slate-50 md:bg-transparent px-3 py-2 md:p-0 rounded-xl mt-1 md:mt-0 border border-slate-100 md:border-transparent">
                      <span className="md:hidden text-[10px] font-black text-slate-400 uppercase">Games:</span>
                      <span className="font-mono font-black text-sm text-slate-700">{totalGames}</span>
                    </div>

                    {/* Total Wins */}
                    <div className="col-span-1 flex md:justify-center justify-between items-center bg-slate-50 md:bg-transparent px-3 py-2 md:p-0 rounded-xl mt-1 md:mt-0 border border-slate-100 md:border-transparent">
                      <span className="md:hidden text-[10px] font-black text-slate-400 uppercase">Wins:</span>
                      <span className="font-mono font-black text-sm text-emerald-600">{totalWins}</span>
                    </div>
